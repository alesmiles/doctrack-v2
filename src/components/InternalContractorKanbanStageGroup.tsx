...
import { InternalContractorKanbanCard } from "@/components/InternalContractorKanbanCard";
import { InternalContractorKanbanDocument, InternalContractorStageKey } from "@/mocks/internalContractorsMockData";

interface Stage {
  key: InternalContractorStageKey;
  title: string;
  documents: InternalContractorKanbanDocument[];
}

interface InternalContractorKanbanStageGroupProps {
  stages: Stage[];
}

const COLUMN_WIDTH = 300;

export function InternalContractorKanbanStageGroup({ stages }: InternalContractorKanbanStageGroupProps) {
        ...
        {stages.map((stage) => (
          ...
          >
            {stage.documents.map((document) => (
              <InternalContractorKanbanCard key={document.id} document={document} />
            ))}
          </div>
        ))}
      </div>
...
export default InternalContractorKanbanStageGroup;
