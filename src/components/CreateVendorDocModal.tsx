...
const VENDORS = ["Блогер Agency", "MediaBuy LLC", "Print Studio"]
const PROJECTS = ["ALF-9 · Март 2025", "ALF-10 · Апрель 2025"]
const DOC_TYPES = ["Договор", "Счёт", "Акт", "Приложение", "УПД"]
const STATUSES = ["Не создан", "На согласовании", "Отправлен ЭДО", "Подписан ЭДО", "Подписан ОРИГ", "Отклонён"]
const MANAGERS = ["Инна Михрабова", "Дмитрий Ларин"]
const CATEGORIES = ["Аренда", "Программное обеспечение", "Хоз. нужды", "Прочее"]
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-[520px] p-0 gap-0 overflow-hidden">
        <DialogHeader className="px-6 pt-5 pb-4 border-b border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-0.5">Создание или регистрация документа по проекту с подрядчиком</p>
        </DialogHeader>
        ...
        <div className="px-6 py-5 overflow-y-auto max-h-[70vh] flex flex-col gap-4">
          <div className="bg-gray-100 rounded-lg p-0.5 flex">
            {(["project", "expense"] as const).map((m) => (
              ...
              >
                {m === "project" ? "Проектный подрядчик" : "Внепроектные расходы"}
              </button>
            ))}
          </div>
          ...
          {mode === "project" && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2.5 mb-2.5">
                <div className="flex flex-col gap-1">
                  ...
                  <Select value={vendor} onValueChange={setVendor}>
                    <SelectTrigger className="h-[34px] text-[13px]"><SelectValue placeholder="Выбрать" /></SelectTrigger>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-md p-2.5 text-[11px] text-gray-600">
                Клиентский документ: Приложение ALF-9-03 · Не подписан — уточните статус перед оплатой
              </div>
            </div>
          )}
