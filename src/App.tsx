...
const STALE_STATUSES: Status[] = ["На согласовании", "Отправлен ОРИГ", "Отправлен ЭДО"];
...
function staleDays(doc: Doc): number | null {
  if (!STALE_STATUSES.includes(doc.status)) return null;
...
const PROJECT_COL_TEMPLATE = "16px 15% 8% 7% 9% 11% 11% 12% 12% 24px";
...
function SortableDocRow({
  ...
  onEstimateChange: (pid: number, did: number, v: number | null) => void;
}) {
  ...
  const [estimateVal, setEstimateVal] = useState(doc.estimate != null ? String(doc.estimate) : "");
  ...
  const commitEstimate = () => {
    ...
    const raw = estimateVal.trim().replace(/\s/g, "").replace(",", ".");
    onEstimateChange(projectId, doc.id, raw === "" ? null : (isNaN(Number(raw)) ? null : Number(raw)));
  };
  ...
  return (
    <tr
      ...
      className={cn(
        ...
        paymentOverdue ? "bg-red-50 hover:bg-red-100/70" : "hover:bg-gray-50/50"
      )}
    >
      {/* Drag handle */}
      <td className="pl-2 pr-1 py-2.5 w-7">
        <button
      ...
      <td className="px-3 py-2.5">
        <span className="inline-flex items-center gap-1">
          <Badge variant="secondary" className="font-normal text-xs bg-gray-100 text-gray-700 hover:bg-gray-100 whitespace-nowrap">
            {doc.type}
          </Badge>
          {stale != null && (
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-gray-200 text-gray-500 text-[9px] font-semibold cursor-default">
                    {stale}
                  </span>
                </TooltipTrigger>
                <TooltipContent>
                  {doc.status === "На согласовании"
                    ? `На согласовании ${stale} дн.`
                    : `${doc.status} ${stale} дн.`}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
      ...
      {/* Оплата план */}
      <td className={cn("px-3 py-2.5 text-sm whitespace-nowrap", paymentOverdue ? "text-red-600 font-medium" : "text-gray-600")}>
        {doc.datePlan ?? "—"}
        {paymentOverdue && days != null && (
          <span className="ml-1 text-[11px] text-red-500">+{days} дн.</span>
        )}
      </td>
      {/* Оплата факт */}
      <td className="px-3 py-2.5 text-sm text-gray-600 whitespace-nowrap">{doc.dateFact ?? "—"}</td>
      {/* Смета — only for Приложение, value = sum */}
      <td className="px-3 py-2.5 text-sm">
        {isApplication ? (
          editEstimate ? (
            <input
              ref={estimateRef}
              className="w-28 border border-blue-300 rounded px-1.5 py-0.5 text-sm outline-none focus:ring-1 focus:ring-blue-500"
              value={estimateVal}
              onChange={(e) => setEstimateVal(e.target.value)}
              onBlur={commitEstimate}
              onKeyDown={(e) => { if (e.key === "Enter") commitEstimate(); }}
              autoFocus
            />
          ) : (
            <button
              className="cursor-pointer rounded px-1.5 py-0.5 -mx-1.5 inline-block min-w-[40px] transition-colors text-left hover:underline underline-offset-2"
              onClick={() => {
                if (doc.estimate != null) {
                  console.log("Открыть смету", doc.estimate);
                  alert(`Открыть смету: ${doc.estimate.toLocaleString("ru-RU")} ₽`);
                } else {
                  setEstimateVal("");
                  setEditEstimate(true);
                  setTimeout(() => estimateRef.current?.focus(), 0);
                }
              }}
              onDoubleClick={() => {
                setEstimateVal(doc.estimate != null ? String(doc.estimate) : "");
                setEditEstimate(true);
                setTimeout(() => estimateRef.current?.focus(), 0);
              }}
            >
              {doc.estimate != null
                ? <span className="text-gray-800">{doc.estimate.toLocaleString("ru-RU")} ₽</span>
...
function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  const sections = [
    {
      ...
      items: [
        ...
        { id: "contractors", icon: Users, label: "Подрядчики" },
      ],
    },
    {
      title: "ПЛАТЕЖИ",
      items: [
        { id: "receivables", icon: DollarSign, label: "Дебиторка" },
        { id: "topay", icon: Mail, label: "На оплату" },
      ],
    },
    {
      title: "БАЗА",
      items: [
        { id: "base-clients", icon: Building2, label: "Клиенты" },
        { id: "base-contractors", icon: Wrench, label: "Подрядчики" },
        { id: "employees", icon: User, label: "Сотрудники" },
      ],
    },
    {
      title: "СИСТЕМА",
  ...
  return (
    <aside className={cn("h-screen flex flex-col border-r border-gray-200 bg-white transition-all duration-200 flex-shrink-0", collapsed ? "w-14" : "w-56")}>
      {/* Logo */}
      <div className={cn("flex items-center h-14 border-b border-gray-100 px-4", collapsed ? "justify-center" : "gap-3")}>
          ...
          <div key={section.title} className="mb-4">
            ...
            {sIdx === 0 && (
              <DropdownMenu>
                ...
                <DropdownMenuContent side={collapsed ? "right" : "bottom"} align="start" className="w-40">
                  <DropdownMenuItem onClick={() => console.log("create project")}>Проект</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => console.log("create document")}>Документ</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            )}
      ...
      <button onClick={onToggle} className="flex items-center justify-center h-10 border-t border-gray-100 text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors">
...
type OptionalFilterId = "direction" | "year" | "doManager";
...
export default function App() {
  ...
  const resetFilters = () => {
    ...
    setDirectionFilter(""); setYearFilter(""); setDoManagerFilter(""); setShowOverdueOnly(false);
  };

  // ── Overdue count (only Счёт) ─────────────────────────────────────────────────
  const totalOverduePayments = projects.reduce(
    (acc, p) => acc + p.documents.filter(isOverduePayment).length, 0
  );
  ...
  const handleEstimateChange = (pid: number, did: number, estimate: number | null) =>
    setProjects((prev) => prev.map((p) => p.id !== pid ? p : {
      ...p, documents: p.documents.map((d) => d.id !== did ? d : { ...d, estimate }),
    }));
    ...
    <div className="flex h-screen overflow-hidden bg-white">
      ...
      <main className="flex-1 overflow-y-auto min-w-0">
        ...
        <div className="flex items-center justify-between px-8 pt-8 pb-4">
          ...
          {hasAnyFilter && (
            <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-700 hover:bg-red-50" onClick={resetFilters}>
        ...
        <div className="px-8 pb-3">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {hasAnyFilter && (
              <Button variant="ghost" size="sm" className="text-blue-600 h-9 flex-shrink-0" onClick={resetFilters}>
                <X className="w-3.5 h-3.5 mr-1" />
                Сбросить
              </Button>
            )}
        ...
        {/* Overdue payments banner — amber/yellow theme */}
        {totalOverduePayments > 0 && (
          <div className="px-8 pb-3">
            <div className="flex items-center gap-3 px-3 py-2 rounded-lg" style={{ backgroundColor: "#FFFBEB", border: "1px solid #FDE68A" }}>
              <AlertTriangle className="w-4 h-4 flex-shrink-0" style={{ color: "#D97706" }} />
              <span className="text-sm font-medium" style={{ color: "#92400E" }}>
                Просрочено оплат: {totalOverduePayments}
              </span>
              <button
                onClick={() => setShowOverdueOnly((p) => !p)}
                className={cn(
                  "ml-auto text-xs px-3 py-1 rounded-full font-medium transition-colors",
                  showOverdueOnly
                    ? "text-white hover:opacity-90"
                    : "border hover:opacity-90"
                )}
                style={showOverdueOnly
                  ? { backgroundColor: "#D97706", color: "#fff" }
                  : { color: "#92400E", borderColor: "#F59E0B", backgroundColor: "transparent" }
                }
              >
                {showOverdueOnly ? "Сбросить" : "Показать просроченные"}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="pb-16">
          ...
          <div className="sticky top-0 bg-white z-10 border-b border-gray-100">
            <div className="px-8 py-2" style={{ display: "grid", gridTemplateColumns: PROJECT_COL_TEMPLATE, alignItems: "center" }}>
              ...
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Проект</span>
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Месяц</span>
                  ...
                  <div key={project.id} className="mb-1">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5">
                        <div className={cn("w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-semibold text-white flex-shrink-0", project.kam.color)}>
                      ...
                      {/* DO Manager */}
                      <div className="flex items-center gap-1.5">
                        <div className={cn("w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-semibold text-white flex-shrink-0", project.doManager.color)}>
                          {project.doManager.initials}
                        </div>
                        <span className="text-sm text-gray-600 truncate">{project.doManager.name}</span>
                      </div>
                      {/* Progress + overdue badge */}
                      <div className="flex items-center gap-2">
                        ...
                        {overduePaymentCount > 0 && (
                          <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-red-500 bg-red-50 px-1.5 py-0.5 rounded-full">
                            <AlertTriangle className="w-2.5 h-2.5" />{overduePaymentCount}
                          </span>
                        )}
                      </div>
                          ...
                          <table className="w-full mt-1">
                            <thead>
                              <tr className="text-[10px] uppercase text-gray-400 tracking-wider">
                                ...
                                <th className="px-3 py-2 text-left font-medium">Оплата план</th>
                                <th className="px-3 py-2 text-left font-medium">Оплата факт</th>
