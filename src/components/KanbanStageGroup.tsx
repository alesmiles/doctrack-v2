...
import { KanbanDocument, KanbanStageKey } from "@/mocks/kanbanMockData";
...
export function KanbanStageGroup({ stages }: KanbanStageGroupProps) {
        ...
        {stages.map((stage) => (
          ...
          >
            {stage.documents.map((document) => (
              <KanbanCard key={document.id} document={document} />
            ))}
          </div>
        ))}
      </div>
...
export default KanbanStageGroup;
